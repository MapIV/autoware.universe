  ...
  const BehaviorPathPlannerParameters & common_parameter, const LaneChangeParameters & parameter)
{
  ...
  for (double acceleration = 0.0; acceleration >= -maximum_deceleration;
    ...
    const PathWithLaneId target_lane_reference_path = getReferencePathFromTargetLane(
      ...
      lane_changing_distance, forward_path_length, lane_changing_speed, num_to_preferred_lane,
  ...
  const double lane_changing_speed, const int & num_to_preferred_lane, const double & minimum_lane_change_length)
{
  ...
  const auto resample_interval =
    std::max((s_end - s_start) / min_resampling_points, lane_changing_speed * resampling_dt);
