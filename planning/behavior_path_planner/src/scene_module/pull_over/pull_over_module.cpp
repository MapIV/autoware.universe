...
void view(T e)
{
  std::cerr << __func__ << ": " << __LINE__ << ", " << e << std::endl;
}
template <typename T>
...
BehaviorModuleOutput PullOverModule::plan()
{
  ...
  if (status_.is_safe) {
    ...
    output.path_candidate = std::make_shared<PathWithLaneId>(status_.pull_over_path.getFullPath());
  } else {
...
BehaviorModuleOutput PullOverModule::planWaitingApproval()
{
  ...
  out.path_candidate = status_.is_safe
...
std::pair<HazardLightsCommand, double> PullOverModule::getHazardInfo() const
{
  ...
  if (
    ...
    status_.pull_over_path.type == PullOverPlannerType::ARC_BACKWARD) {
